...
date_dim['quarter_start'] = date_dim['date'].dt.to_period('Q').apply(lambda p: p.start_time)

date_dim['day_of_quarter'] = (date_dim['date'] - date_dim['quarter_start']).dt.days + 1
date_dim.drop('quarter_start', axis=1, inplace=True)
...
# Inspecting tables present in the database
inspector = db.inspect(engine)
tables = inspector.get_table_names()
print(tables)

# Removing member table:
